...
describe('ChangePassword', () => {
    ...
    it('validates password correctly', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText(/password should contain at least one special character/i)).toBeInTheDocument();
        });
        ...
        fireEvent.change(passwordInput, { target: { value: 'LongEnough1!' } });
        await waitFor(() => {
            expect(screen.queryByText(/password should contain at least one special character/i)).not.toBeInTheDocument();
            expect(screen.getByText(/password should contain at least two numerical digits/i)).toBeInTheDocument();
        });
    ...
    it('validates confirm password correctly', async () => {
        ...
        confirmPasswordInputs.forEach((confirmPasswordInput) => {
            ...
            fireEvent.change(confirmPasswordInput, { target: { value: 'DifferentPassword12!' } });
            waitFor(() => {
            expect(screen.getByText(/passwords do not match/i)).toBeInTheDocument();
        });
            fireEvent.change(confirmPasswordInput, { target: { value: 'ValidPassword12!' } });
            waitFor(()=>{
    ...
    it('submits form successfully', async () => {
        ...
        await waitFor(() => expect(axiosInstance.post).toHaveBeenCalledWith('auth/change-password', { newPassword: 'ValidPassword12!' }));
    ...
    it('handles submit failure', async () => {
        ...
        (axiosInstance.post as jest.Mock).mockRejectedValue(new Error('Failed to change password.'));

        const resetPasswordButtons = screen.queryAllByRole('button', { name: /Reset Password/i });
