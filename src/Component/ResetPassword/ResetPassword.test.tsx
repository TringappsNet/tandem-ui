...
describe('Reset', () => {
    ...
    test('displays error message on password mismatch', async () => {
        ...
        fireEvent.change(screen.getByLabelText('Confirm New Password:'), { target: { value: 'MismatchedPassword' } });
    ...
    test('displays error message on invalid password', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText('Password must contain at least 1 special character, 1 number, and 1 capital letter.')).toBeInTheDocument();
            expect(axiosInstance.post).not.toHaveBeenCalled();
        });
    });

    test('displays appropriate error message on API failure', async () => {
        (axiosInstance.post as jest.Mock).mockRejectedValueOnce({ message: 'Internal Server Error' });
    ...
    test('resets form and closes dialog on successful password reset', async () => {
        (axiosInstance.post as jest.Mock).mockResolvedValueOnce({ status: 200, data: { message: 'Password reset successful.' } });
        ...
        await waitFor(() => {
            ...
            setTimeout(() => {
                ...
                expect(onCloseDialogMock).toHaveBeenCalled(); }, 1000);
        });
    });
});
